...
function isRNUrl(url) {
  // Return whether the URL starts with the Gmail prefix.
  return url.indexOf(getRoadNinjaUrl()) == 0;
}
...
function goToRoadNinjaHome() {
  console.log('Going to inbox...');
  chrome.tabs.getAllInWindow(undefined, function(tabs) {
    for (var i = 0, tab; tab = tabs[i]; i++) {
      if (tab.url && isRNUrl(tab.url)) {
    ...
    chrome.tabs.create({url: getRoadNinjaUrl()});
  });
}
...
var filters = {
  ...
  url: [{urlContains: getRoadNinjaUrl().replace(/^https?\:\/\//, '')}]
};

function onNavigate(details) {
  if (details.url && isRNUrl(details.url)) {
...
chrome.browserAction.onClicked.addListener(goToRoadNinjaHome);
